...
def Lock(con, cur):
    ...
    while True:
        ...
        if (lock_369b == 1 and unlock_369b == 0):
            ...
            error_369b = LaserLock_369b.update(freq[1])
            #print ("error 369 = ",error_369)
        else:
        ...
        if (lock_399 != 0 and unlock_399 == 0):
            ...
            error_399 = LaserLock_399.update(freq[2])
        ...
        if (lock_935 != 0 and unlock_935 == 0):
            ...
            error_935 = LaserLock_935.update(freq[3])
        ...
        cur.execute("INSERT INTO `wavemeter`.`error` (time, `369`, `369b`, `399`, `935`, 369w, 369bw, 399w, 935w) VALUES (\'%s\',\'%s\',\'%s\',\'%s\',\'%s\',\'%s\',\'%s\',\'%s\',\'%s\');",(cTime,round(error_369,4), round(error_369b,4), round(error_399,4), round(error_935,4), freq[0], freq[1], freq[2], freq[3]))
...
con = mdb.connect(host = '127.0.0.1', user = 'python',passwd = 'abc123',db = 'wavemeter',port = 3306, connect_timeout = 1000)
cur = con.cursor()
