...
def Lock(con, cur):
    ...
    LaserLock_369 = PID(P=200, I=400, D=0)
    LaserLock_399 = PID(P=300, I=300, D=0)
    LaserLock_935 = PID(P=350, I=500, D=0)
    ...
    ADDA935=setVoltage.SetVoltage(offset_935, "PXI1Slot2/ao2")# to define a single thread outside the while cycle
    ...
    while True:
        ...
        #time.sleep(.3)
        if (np.absolute(error_369)>=10) or (np.absolute(error_399)>=10) or (np.absolute(error_935)>=10):
